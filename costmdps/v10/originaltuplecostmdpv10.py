...
class OriginalTupleMDP(object):
    ...
    def populateMatrices(self):
        for state_index in range(self.state_count):
            ...
            self.reward_selfish[ADOPT][state_index, self.state_mapping[1, 0, IRRELEVANT]] = - self.mining_cost * self.alpha
            self.reward_selfish[ADOPT][state_index, self.state_mapping[0, 1, IRRELEVANT]] = - self.mining_cost * self.alpha
            ...
            if a > h:
                ...
                self.reward_selfish[OVERRIDE][state_index, self.state_mapping[a-h, 0, IRRELEVANT]] = h + 1 - self.mining_cost * self.alpha
                self.reward_selfish[OVERRIDE][state_index, self.state_mapping[a-h-1, 1, RELEVANT]] = h + 1 - self.mining_cost * self.alpha
            else:
            ...
            if (fork != ACTIVE) and (a < self.T) and (h < self.T):
                ...
                self.reward_selfish[WAIT][state_index, self.state_mapping[a+1, h, IRRELEVANT]] = - self.mining_cost * self.alpha
                self.reward_selfish[WAIT][state_index, self.state_mapping[a, h+1, RELEVANT]] = - self.mining_cost * self.alpha
                ...
                self.reward_selfish[WAIT][state_index, self.state_mapping[a+1, h, ACTIVE]] = - self.mining_cost * self.alpha
                self.reward_selfish[WAIT][state_index, self.state_mapping[a-h, 1, RELEVANT]] = h - self.mining_cost * self.alpha
                self.reward_selfish[WAIT][state_index, self.state_mapping[a, h+1, RELEVANT]] = - self.mining_cost * self.alpha
            else:
            ...
            if (fork == RELEVANT) and (a >= h) and (h > 0) and (a < self.T) and (h < self.T):
                ...
                self.reward_selfish[MATCH][state_index, self.state_mapping[a+1, h, ACTIVE]] = - self.mining_cost * self.alpha
                self.reward_selfish[MATCH][state_index, self.state_mapping[a-h, 1, RELEVANT]] = h - self.mining_cost * self.alpha
                self.reward_selfish[MATCH][state_index, self.state_mapping[a, h+1, RELEVANT]] = - self.mining_cost * self.alpha
            else:
...
if __name__ == "__main__":
    ...
    T = 8
    mining_cost = 0.5
    original_mdp = OriginalTupleMDP(alpha=alpha, gamma=gamma, T=T, mining_cost=mining_cost, epsilon=10e-5)
