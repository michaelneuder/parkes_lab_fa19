...
class OriginalCostMDP(object):
    ...
    def populateMatrices(self):
        for state_index in range(self.state_count):
            ...
            # adopt rewards
            self.rewards[ADOPT][state_index, self.state_mapping[1, 0, IRRELEVANT]] = -h - self.mining_cost * self.alpha
            self.rewards[ADOPT][state_index, self.state_mapping[0, 1, IRRELEVANT]] = -h - self.mining_cost * self.alpha
            ...
            if (fork != ACTIVE) and (a < self.T) and (h < self.T):
                ...
                self.rewards[WAIT][state_index, self.state_mapping[a+1, h, IRRELEVANT]] = - self.mining_cost * self.alpha
