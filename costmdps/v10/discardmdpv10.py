...
class OriginalCostMDP(object):
    ...
    def populateMatrices(self):
        for state_index in range(self.state_count):
            ...
            # adopt rewards
            self.rewards[ADOPT][state_index, self.state_mapping[1, 0, IRRELEVANT]] = -h
            self.rewards[ADOPT][state_index, self.state_mapping[0, 1, IRRELEVANT]] = -h
            ...
            if a > h:
                ...
                self.rewards[OVERRIDE][state_index, self.state_mapping[a-h, 0, IRRELEVANT]] = h+1
                self.rewards[OVERRIDE][state_index, self.state_mapping[a-h-1, 1, RELEVANT]] = h+1
            else:
            ...
            if (fork != ACTIVE) and (a < self.T) and (h < self.T):
                ...
                self.rewards[WAIT][state_index, self.state_mapping[a-h, 1, RELEVANT]] = h
            else:
            ...
            if (fork == RELEVANT) and (a >= h) and (h > 0) and (a < self.T) and (h < self.T):
                ...
                self.rewards[MATCH][state_index, self.state_mapping[a-h, 1, RELEVANT]] = h
            else:
