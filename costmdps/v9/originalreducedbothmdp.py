...
class OriginalReducedMDP(object):
    ...
    def populateMatrices(self):
        for state_index in range(self.state_count):
            ...
            # adopt rewards
            self.reward_honest[ADOPT][state_index, self.state_mapping[1, 0]] = h-1 * self.beta * self.mining_cost 
            self.reward_honest[ADOPT][state_index, self.state_mapping[0, 1]] = h-1 * self.beta * self.mining_cost 
            self.reward_selfish[ADOPT][state_index, self.state_mapping[1, 0]] = -1 * self.alpha * self.mining_cost 
            self.reward_selfish[ADOPT][state_index, self.state_mapping[0, 1]] = -1 * self.alpha * self.mining_cost
            
            # override
            if a > h:
                self.transitions[OVERRIDE][state_index, self.state_mapping[a-h, 0]] = self.alpha
                self.reward_selfish[OVERRIDE][state_index, self.state_mapping[a-h, 0]] = h+1 - self.alpha * self.mining_cost
                self.reward_honest[OVERRIDE][state_index, self.state_mapping[a-h, 0]] = - self.beta * self.mining_cost
                self.transitions[OVERRIDE][state_index, self.state_mapping[a-h-1, 1]] = 1 - self.alpha
                self.reward_selfish[OVERRIDE][state_index, self.state_mapping[a-h-1, 1]] = h+1 - self.alpha * self.mining_cost
                self.reward_honest[OVERRIDE][state_index, self.state_mapping[a-h-1, 1]] = - self.beta * self.mining_cost
            else:
            ...
            if (a < self.T) and (h < self.T):
                ...
                self.reward_selfish[WAIT][state_index, self.state_mapping[a+1, h]] = -1 * self.alpha * self.mining_cost
                self.reward_selfish[WAIT][state_index, self.state_mapping[a, h+1]] = -1 * self.alpha * self.mining_cost
                self.reward_honest[WAIT][state_index, self.state_mapping[a+1, h]] = -1 * self.beta * self.mining_cost
                self.reward_honest[WAIT][state_index, self.state_mapping[a, h+1]] = -1 * self.beta * self.mining_cost
            else:
...
if __name__ == "__main__":
    ...
    T = 8
    mining_cost = 0
    original_mdp = OriginalReducedMDP(alpha=alpha, T=T, mining_cost=mining_cost, epsilon=10e-5)
