    ...
    def populateMatrices(self):
        for state_index in range(self.state_count):
            ...
            if (fork != ACTIVE) and (a < self.T) and (h < self.T):
                ...
                self.transitions[MINE][state_index, self.state_mapping[a-h, 1, RELEVANT]] = self.gamma*(1-self.alpha)
                self.reward_selfish[MINE][state_index, self.state_mapping[a-h, 1, RELEVANT]] = h
                self.transitions[MINE][state_index, self.state_mapping[a, h+1, RELEVANT]] = (1-self.gamma)*(1-self.alpha)
            else:
                self.transitions[MINE][state_index, 0] = 1
                self.reward_honest[MINE][state_index, 0] = 10000
